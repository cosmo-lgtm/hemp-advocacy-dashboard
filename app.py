...
def load_all_data():
    ...
    data['production'] = load_data("""
        SELECT * FROM `artful-logic-475116-p1.hemp_advocacy.production_by_state`
        ORDER BY year, hemp_type
    """)

    data['market'] = load_data("""
        SELECT * FROM `artful-logic-475116-p1.hemp_advocacy.market_metrics`
        ORDER BY year
    """)

    data['employment'] = load_data("""
        SELECT * FROM `artful-logic-475116-p1.hemp_advocacy.employment_stats`
        ORDER BY year
    """)

    data['regulatory'] = load_data("""
        SELECT * FROM `artful-logic-475116-p1.hemp_advocacy.regulatory_status`
        ORDER BY state
    """)

    data['tax'] = load_data("""
        SELECT * FROM `artful-logic-475116-p1.hemp_advocacy.tax_revenue`
        ORDER BY year, state
    """)

    data['timeline'] = load_data("""
        SELECT * FROM `artful-logic-475116-p1.hemp_advocacy.industry_timeline`
        ORDER BY event_date
    """)
...
<div style="text-align: center; padding: 20px 0 30px 0;">
    <h1 style="color: #1a472a; font-size: 2.5rem; margin-bottom: 8px;">
...
with col1:
    ...
    <div class="metric-card">
        <div class="metric-value">$445M</div>
        <div class="metric-label">2024 Production Value</div>
        <div class="metric-change">↑ 40% from 2023</div>
    </div>
    """, unsafe_allow_html=True)
...
with col2:
    ...
    <div class="metric-card">
        ...
        <div class="metric-label">Jobs Supported</div>
        <div class="metric-change">↑ 5.4% YoY growth</div>
    </div>
    """, unsafe_allow_html=True)
...
with col3:
    ...
    <div class="metric-card">
        ...
        <div class="metric-label">State Tax Revenue</div>
        <div class="metric-change">2024 Collections</div>
    </div>
    """, unsafe_allow_html=True)
...
with col4:
    st.markdown("""
    <div class="metric-card">
        <div class="metric-value">45,294</div>
        <div class="metric-label">Acres Planted</div>
        <div class="metric-change">↑ 64% from 2023</div>
    </div>
    """, unsafe_allow_html=True)
...
with col1:
    ...
    fig.add_trace(go.Bar(
        ...
        textfont=dict(size=14, color='#1a472a')
    ))
    fig.update_layout(
...
with col2:
    ...
    market_df = data['market'][
        (data['market']['metric_name'] == 'US Industrial Hemp Market') |
        (data['market']['metric_name'] == 'Cannabis Beverages Market')
    ].copy()
    ...
    fig.update_layout(
        title="U.S. Industrial Hemp Market Trajectory",
        xaxis_title="Year",
...
with col1:
    ...
    status_colors = {
        'legal': '#2d5a3d',
        'legal_restricted': '#6b8e23',
        'pending': '#daa520',
        'dispensary_only': '#cd853f',
        'banned': '#8b0000'
    }
    ...
    for status, count in status_counts.items():
        ...
        <div style="display: flex; align-items: center; margin: 8px 0;">
            ...
            <span><strong>{count}</strong> states - {label}</span>
        </div>
        """, unsafe_allow_html=True)
...
with col2:
    ...
    fig = go.Figure(data=go.Choropleth(
        ...
        colorscale=[
            [0, '#8b0000'],      # banned
            [0.25, '#cd853f'],   # dispensary_only
            [0.5, '#daa520'],    # pending
            [0.75, '#6b8e23'],   # legal_restricted
            [1, '#2d5a3d']       # legal
        ],
        showscale=False,
...
with col3:
    ...
    st.markdown("""
    - **28 states** have legal THC beverage sales
...
with col1:
    ...
    fig.update_layout(
        title="U.S. Cannabis/Hemp Industry Employment",
        xaxis_title="Year",
...
with col2:
    # Tax revenue by top states
    tax_states = data['tax'][
        (data['tax']['state'] != 'US') &
        (data['tax']['year'] == 2023) &
        (data['tax']['quarter'] == 4)
    ].nlargest(8, 'tax_revenue_usd')
    ...
    fig.update_layout(
        title="Quarterly State Tax Revenue (Q4 2023)",
        xaxis_title="Tax Revenue ($ Millions)",
        yaxis_title="",
        showlegend=False,
        plot_bgcolor='white',
        height=350,
...
with col1:
    ...
    <div style="background: #f8f9fa; padding: 24px; border-radius: 12px; text-align: center;">
        <div style="font-size: 3rem; font-weight: 700; color: #1a472a;">64%</div>
        <div style="color: #666;">of Americans are familiar with CBD products</div>
    </div>
    """, unsafe_allow_html=True)
...
with col2:
    st.markdown("""
    <div style="background: #f8f9fa; padding: 24px; border-radius: 12px; text-align: center;">
        <div style="font-size: 3rem; font-weight: 700; color: #1a472a;">36.5%</div>
        <div style="color: #666;">CBD beverage sales growth in 2023</div>
    </div>
    """, unsafe_allow_html=True)
...
with col3:
    st.markdown("""
    <div style="background: #f8f9fa; padding: 24px; border-radius: 12px; text-align: center;">
        <div style="font-size: 3rem; font-weight: 700; color: #1a472a;">70%</div>
        <div style="color: #666;">Millennials/Gen Z willing to pay premium for traceable products</div>
    </div>
    """, unsafe_allow_html=True)
...
for _, row in timeline_df.iterrows():
    ...
    st.markdown(f"""
    <div style="display: flex; margin: 12px 0; padding: 12px; background: #f8f9fa; border-radius: 8px; border-left: 4px solid {impact_color};">
        <div style="min-width: 100px; font-weight: 600; color: #666;">{row['event_date'].strftime('%b %Y')}</div>
        <div>
            <div style="font-weight: 600; color: #1a472a;">{row['title']}</div>
            <div style="color: #666; font-size: 0.9rem;">{row['description']}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
<div style="text-align: center; color: #666; padding: 20px;">
    <p><strong>Data Sources:</strong> USDA NASS, U.S. Census Bureau, Vangst, Grand View Research, MultiState, Vicente LLP, Whitney Economics</p>
    <p style="font-size: 0.8rem;">Last updated: November 2025 | Data refreshed quarterly from public sources</p>
</div>
""", unsafe_allow_html=True)
